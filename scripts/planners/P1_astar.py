...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        return x[1] >=self.statespace_lo[1] and x[0] >= self.statespace_lo[0] and \
            x[1] <= self.statespace_hi[1] and x[0] <= self.statespace_hi[0] and self.occupancy.is_free(x)
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        for angle in angles:
            point = self.snap_to_grid((x[0]+self.resolution*np.cos(angle),x[1]+self.resolution*np.sin(angle) ))
    ...
    def solve(self):
        ...
        i = 0
        max_iter = 10000
        while len(self.open_set) > 0 and max_iter > i:
            i+=1
            ...
            for neighbor in self.get_neighbors(x_curr):
                if neighbor in self.closed_set: 
                    continue
                tentative_cost = self.cost_to_arrive[x_curr] + self.distance(x_curr, neighbor)
